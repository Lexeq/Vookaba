...
--set creation date
CREATE FUNCTION public.set_creation_date()
RETURNS TRIGGER AS $$
BEGIN
    NEW."Created" = now();
    RETURN NEW;
...
CREATE TRIGGER set_post_creation_time
BEFORE INSERT on public."Posts"
FOR EACH ROW
WHEN (NEW."Created" IS NULL)
EXECUTE FUNCTION public.set_creation_date();

CREATE TRIGGER set_token_creation_time
BEFORE INSERT on public."AnonymousTokens"
FOR EACH ROW
WHEN (NEW."Created" IS NULL)
EXECUTE FUNCTION public.set_creation_date();

CREATE TRIGGER set_report_creation_time
...
--set thread timestamps
CREATE FUNCTION public.set_thread_times()
RETURNS TRIGGER AS $$
BEGIN
    ...
    THEN
        NEW."Created" =  now();
    END IF;
    IF(NEW."LastBump" IS NULL)
    THEN
        NEW."LastBump" = now();
    END IF;
    IF(NEW."LastHit" IS NULL)
    THEN
        NEW."LastHit" =  now();
    END IF;
    RETURN NEW;
...
CREATE TRIGGER set_thread_times
BEFORE INSERT on public."Threads"
FOR EACH ROW
EXECUTE FUNCTION public.set_thread_times();
